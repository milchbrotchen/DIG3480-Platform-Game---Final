...
var actorChars = {
  "@": Player,
  "o": Peach, // A coin will wobble up and down
  "g": Ghost,
  "s": Seal,
  "*": Gate,
  "=": Hands, "|": Hands, "v": Hands
...
var imgArea = document.getElementById("health");

console.log("Temple of the Manes Kevin MacLeod (incompetech.com) \nLicensed under Creative Commons: By Attribution 3.0 License \n http://creativecommons.org/licenses/by/3.0/");
...
Peach.prototype.type = "peach";

function Ghost(pos){
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(.55, .95);
...
// Hands is initialized based on the character, but otherwise has a
// size and position
function Hands(pos, ch) {
  this.pos = pos;
  ...
  if (ch == "=") {
    ...
    // Drip Hands. Repeat back to this pos.
    this.speed = new Vector(0, 3);
    this.repeatPos = pos;
  }
}
Hands.prototype.type = "hands";
...
Hands.prototype.act = function(step, level) {
...
Peach.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

Seal.prototype.act = function(step){
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
}
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // if the player touches Hands and the player hasn't won
  // Player loses
  if (type == "hands" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
    peachScore = 1;
    updateScore();
    ...
    //touching a ghost will take away a peach and send the player back to start
    //if player runs out of peaches, they lose
    peachScore--;
    if (peachScore >= 1){
      this.player.pos = new Vector (2, 5);
      updateScore();
    }
    else if (peachScore <= 0){
      this.status = "lost";
      this.finishDelay = 1;
    }
  }
    ...
    //if player collects the seal and touches the gate
    if (!this.actors.some(function(actor){
      return actor.type == "seal";
    })) {
      this.status = "won";
      this.finishDelay = .01;
    }
